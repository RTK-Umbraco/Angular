...
import { CarResource } from '../interfaces/car-resource';

export class CarService extends DataSource<CarResource> {

  private _dataStream = new ReplaySubject<CarResource[]>();

   cars: Array<CarResource> = [
  ...
  private carsSubject$: Subject<CarResource[]> = new BehaviorSubject<CarResource[]>(this.cars);
  cars$: Observable<CarResource[]> = this.carsSubject$.asObservable();
  ...
  // constructor(initialData: CarResource[]) {
  //   super();
  //   this.setData(initialData);
  // }

  connect(): Observable<CarResource[]> {
    return this._dataStream;
  }

  disconnect() {}

  setData(data: CarResource[]) : Observable<CarResource[]> {
    // this._dataStream.next(data);
    return of(data);
  }

  addData(data: CarResource){
    this.cars.push(data);
  }

  getData(): Observable<CarResource[]>{
    return of(this.cars)
  }
}
